...
import {
  ArrowLeft, MapPin, FileText,
  MessageCircle, Send, Calendar, Clock,
} from 'lucide-react';
...
function relativeTime(dateStr: string): string {
  ...
  // Lebih dari 7 hari → tanggal lengkap
...
export default function ReportDetailPage() {
  ...
  // FIX 4: Auto-update timestamp setiap 60 detik
      ...
      <div className="px-4 space-y-4">
        ...
        {photos.length > 0 && (
          <div className="card overflow-hidden">
            <img
              src={toAbsolute(photos[currentPhoto])}
              alt="Report"
              ...
              <div className="flex gap-1.5 p-2 overflow-x-auto">
                {photos.map((p, i) => (
                  <img
                    ...
                    className={`
                      w-14 h-14 rounded-xl object-cover flex-shrink-0 cursor-pointer
                      border-2 transition-all
                      ${i === currentPhoto ? 'border-brand' : 'border-transparent opacity-70'}
                    `}
                  />
                ))}
              </div>
        ...
        <div className="card p-4">
            ...
            <Field
              ...
              value={
                <span className="flex items-center gap-1 justify-end">
                  ...
                  {new Date((report as any).pickup_date).toLocaleDateString('id-ID', {
          ...
          <Field
            label="Dikirim"
            value={new Date(report.created_at).toLocaleDateString('id-ID', { dateStyle: 'long' })}
          />
          {report.validated_at && (
            <Field
              label="Divalidasi"
              value={new Date(report.validated_at).toLocaleDateString('id-ID', { dateStyle: 'long' })}
            />
          )}
        </div>
        ...
        {report.status === 'approved' && report.net_points != null && (
          <div className="card p-4 bg-brand-50 border-brand-100">
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-ink-muted">Biaya Penanganan</span>
        ...
        {report.status === 'rejected' && (
          <div className="card p-4 bg-red-50 border-red-100">
            ...
            <p className="text-sm text-red-600">
              {report.rejection_reason ?? 'Tidak ada alasan'}
            </p>
          </div>
        )}
        ...
        {report.notes && (
          <div className="card p-4 flex items-start gap-3">
            <FileText size={16} className="text-ink-faint flex-shrink-0 mt-0.5" />
            <div>
        ...
        <div className="card overflow-hidden">
          <div className="px-4 py-3 border-b border-surface-border flex items-center gap-2">
            ...
            <h3 className="font-semibold text-ink text-sm">
              {t('history.commentsTitle') || 'Komentar dengan Admin'}
            </h3>
            <span className="ml-auto text-xs text-ink-muted">
              {(comments as any[]).length} komentar
            </span>
          </div>
          ...
          <div className="px-4 py-3 space-y-3 max-h-64 overflow-y-auto">
            ...
            {!commentsLoading && (comments as any[]).length === 0 && (
              <div className="py-6 text-center">
                <MessageCircle size={20} className="text-ink-faint mx-auto mb-2" />
                <p className="text-xs text-ink-muted">
                  {t('history.noComments') || 'Belum ada komentar. Tanya admin tentang laporanmu.'}
                </p>
              </div>
            )}

            {!commentsLoading && (comments as any[]).map((c: any) => {
              ...
              return (
                ...
                >
                  ...
                  `}>
                    {/* Profile picture kalau ada */}
                    {c.author_avatar ? (
                      <img
                        src={c.author_avatar.startsWith('/uploads/')
                          ? `${API_URL}${c.author_avatar}`
                          : c.author_avatar
                        }
                        alt=""
                      ...
                      isAdmin ? '🛡' : (c.author_name?.[0]?.toUpperCase() ?? 'W')
                    )}
                  </div>
                  ...
                  `}>
                    ...
                    {/* ── FIX 4: Relative timestamp ─────────────────── */}
                    <p className={`text-[10px] mt-1 ${isMe ? 'text-white/60' : 'text-ink-faint'}`}>
          ...
          <div className="px-4 pb-4 border-t border-surface-border pt-3">
            <div className="flex gap-2 items-end">
              <textarea
                ...
                placeholder={t('history.commentPlaceholder') || 'Tanya admin... (Enter untuk kirim)'}
