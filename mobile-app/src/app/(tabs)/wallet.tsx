...
export default function WalletScreen() {
  ...
  async function handleWithdraw() {
    ...
    if (!form.amount || isNaN(amt) || amt <= 0) {
      show(t("wallet.validAmount") || "Enter valid amount", "error");
      return;
    }
    if (amt > balance) {
      show(t("wallet.insufficientBalance"), "error");
      return;
    }
    if (!form.bank_name || !form.account_number || !form.account_holder) {
      show(t("wallet.allRequired") || "All fields required", "error");
      return;
    }
    try {
              ...
              {[
                {
                  ...
                  placeholder: `max ${balance.toLocaleString("id-ID")}`,
                },
                {
